# -*- coding: utf-8 -*-
"""
Created on Wed Jun 13 16:48:41 2018
...
def record_user_click(index,keyword,url):
    for entry in index:
        if entry[0] == keyword:
        #[keyword,[[url1,click1], [url2,click2]]]
            for urlList in entry[1]:
                if url in urlList:
                    urlList[1] += 1
                    break
    return index
...
def add_to_index(index, keyword, url):
    ...
    for entry in index:
        if entry[0] == keyword:
            for urlList in entry[1]:#official: for ...
                if url not in urlList:              #if url == urlList[0]: 
                    newUrl = True                         #return
                elif url in urlList:           #entry[1].append([url,0])
                    newUrl = False             #return
            if newUrl:
